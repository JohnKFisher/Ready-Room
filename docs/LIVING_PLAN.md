...
- duplicate all-day calendar copies from multiple EventKit calendars now collapse into one normalized timeline item when owner, covered day range, and normalized title line up, while still surfacing the contributing calendar labels for inspection
- the packaging script now builds a derived bundle `Info.plist` instead of rewriting tracked version files on every build, and intentional version/build changes now go through an explicit metadata-update script
